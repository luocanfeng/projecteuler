...
def f():
    _text = open('../data/p059_cipher.txt').read()
    encryptedCodes = list(map(int, _text.split(",")))
    crack(encryptedCodes, itertools.product(range(97, 123), repeat=3), ' the ')

def crack(encryptedCodes, keys, morsel):
    possibilities = []
    for key in keys:
        message = ''.join(map(chr, [x ^ y for x, y in zip(encryptedCodes, itertools.cycle(key))]))
        if morsel in message:
            possibilities.append(message)
#             print(message)
